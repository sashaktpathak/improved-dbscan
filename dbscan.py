    ...
    for p in newNeighbours:
        ...
        if p.IsVisited() != True:
            ...
            if status == 1:
                ...
                cluster1 = Cluster()
                cluster1 = expandClusterOld(p, neighbourCheck, cluster1, eps, minPts , distanceMatrix, eps_, clusters)
...
def DBSCAN(eps, minPts):
    ...
    for point in points:
        ...
        if len(neighbours) < minPts:
            ...
            point.SetType('CORE')
            cluster = Cluster()
            cluster = expandClusterOld(point, neighbours, cluster, eps, minPts, distanceMatrix)
